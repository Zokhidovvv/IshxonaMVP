...
@router.put("/{log_id}", response_model=SkochLogOut)
def update_skoch_log(log_id: int, data: SkochLogCreate, db: Session = Depends(get_db), _=Depends(require_role("admin", "sales", "boss"))):
...
def delete_skoch_log(log_id: int, db: Session = Depends(get_db), _=Depends(require_role("admin", "sales", "boss"))):
    log = db.query(SkochLog).filter(SkochLog.id == log_id).first()
    if not log:
        raise HTTPException(404, "Yozuv topilmadi")
